...
st.write('Please upload the proofs in the first uploader and the print file folder in the second uploader')

proofs_data = st.file_uploader("Upload a PDF", type=["pdf"])
print_data = st.file_uploader("Upload a Text File Folder")
state_data = st.file_uploader("Upload the State COntact Info Sheet", type=["xls"])
...
if proofs_data and print_data and state_data is not None:
    for i in glob.glob(print_data):
        lines = []
        with open(i) as f:
            lines = f.readlines()
